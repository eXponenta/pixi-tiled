...
class TilesetManager extends PIXI$1.utils.EventEmitter {
    ...
    _cropTile(set, tile, texture) {
        ...
        texture = new PIXI$1.Texture(texture.baseTexture, new PIXI$1.Rectangle(margin + xId * (set.tilewidth + space), margin + yId * (set.tileheight + space), set.tileheight, set.tilewidth));
    ...
    _tryLoadTexture(url, tile) {
        ...
        const texture = new PIXI$1.Texture(new PIXI$1.BaseTexture(res));
        PIXI$1.Texture.addToCache(texture, url);
        this._loadQueue++;
        res.load()
            .then(() => {
            texture.emit('loaded');
